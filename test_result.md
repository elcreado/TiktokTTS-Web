...
agent_communication:
    ...
    - agent: "testing"
      message: "🚨 CRITICAL DISCOVERY: TTS MESSAGE DUPLICATION ROOT CAUSE IDENTIFIED! After comprehensive testing with detailed console log analysis, I've identified the exact source of the 'messages playing twice' issue. **THE PROBLEM IS NOT IN THE FRONTEND TTS QUEUE** - it's working perfectly. **ROOT CAUSE: Backend WebSocket is sending duplicate chat messages.** Evidence: Same messages received multiple times with different IDs (e.g., 'doblesss' by JOANY ALLISON: ID 1753931993578.8862 and ID 1753931993643.1633). The frontend TTS queue correctly processes each duplicate as a separate message, causing users to hear the same message twice. **SOLUTION REQUIRED: Fix backend WebSocket message deduplication logic.** Frontend TTS functionality is working as designed - the issue is upstream data duplication from the backend WebSocket broadcasting system. All frontend UI components are functional and working correctly."